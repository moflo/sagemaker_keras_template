...
def test():
    print(
        "Download CIFAR10 data, natively on local machine."
    )
    ...
    if os.path.isdir(data_dir) == False:
        print(
            "Saving Keras training and test data locally..."
        )
        ...
        np.save(
            os.path.join(train_dir, "x_train.npy"), x_train
        )
        np.save(
            os.path.join(train_dir, "y_train.npy"), y_train
        )
...
def main(train_inst):
    print(
        "Download CIFAR10 data, start Keras training on *{}*.".format(
            train_inst
        )
    )
    ...
    val_dir = os.path.join(os.getcwd(), "data/test")
    if (
        os.path.isdir(data_dir) == False
        or os.path.isdir(train_dir) == False
        or os.path.isdir(val_dir) == False
    ):
        print(
            'Error: need to first save local training data. Run with "--train-inst test" first'
        )
        return
    ...
    if train_inst == "local" or train_inst == "local-gpu":
        try:
            ...
            print(
                'Error: Docker is needed to run Sagemaker locally with "local" or "local-gpu" options.'
            )
            return
    ...
    print(
        'Preparing training data on s3 at "{}"...'.format(
            s3_prefix
        )
    )
    ...
    key = "{}/data/train/y_train.npy".format(
        s3_prefix
    )  # Note: need exhaustive check?
    objs = list(bucket.objects.filter(Prefix=key))
    if len(objs) == 4 and objs[0].key == key:
        print("Found existing files in default bucket:")
        for t in objs:
            print(" {}".format(t.key))
        train_saved = "s3://{}/{}/data/train".format(
            bucket_default, s3_prefix
        )
        val_saved = "s3://{}/{}/data/val".format(
            bucket_default, s3_prefix
        )
        ...
        val_s3 = sagemaker.Session().upload_data(
            path="./data/test/",
    ...
    local_estimator = TensorFlow(
        entry_point="cifar10_keras_sage_train.py",
        source_dir=".",
        model_dir=model_dir,
    ...
    print(
        "\nBegining training with parameters {}...".format(
            hyperparameters
        )
    )
